...
for ii = 1:nset
    ...
    for ll = 1:max_eig
        ...
        for kk = 1:n_acc
            ...
            G_num{ii}{ll}(:, kk) = ((A(kk).*(-1.*(omega_M{ii}{ll}).^2))./(-(omega_M{ii}{ll}).^2 + 2*1i*csi*w.*(omega_M{ii}{ll}) + w.^2)) + ((Rl_re(kk) + 1i*Rl_im(kk))./(omega_M{ii}{ll}).^2) + (Rh_re(kk) + 1i*Rh_im(kk)).';
        end
    end
...
for ii = 1:nset
    ...
    for ll = 1:max_eig
        for kk = 1:n_acc
            ...
            G_num_overall{ii}{ll}(:, kk) = (((A(kk).*(-(omega).^2))./(-1.*(omega).^2 + 2*1i*csi*w.*(omega) + w.^2)) ).';
...
%% MAC between numerical and experimental mode shapes
ii = idx_set;
jj = idx_acc;
phi_colocated = zeros(1, max_eig);
phi_non_colocated = zeros(n_acc - 1, max_eig);

% start with the co-located 
for ll = 1:max_eig
    A_colocated = Final_param{ii}{ll}(5);
    A_noncolocated = [Final_param{ii}{ll}(3:4); Final_param{ii}{ll}(6:7)];
    phi_colocated(ll) = sqrt(A_colocated);
    phi_non_colocated(:, ll) = A_noncolocated./phi_colocated(ll);
end

exp_modes_shape = [phi_non_colocated(1:2, :); phi_colocated; phi_non_colocated(3:4, :)];


%% MAC final computation
% importing numerical mode shapes
...
for ii = 1 : max_eig
    ...
    Norm_MDOF_modes(:, ii) = SDOF_modes(:, ii)./max(abs(SDOF_modes(:, ii)));
end
...
for ii = 1:max_eig
    ...
    numerator = abs(Experimental_modes(:, ii)' * SDOF_modes(:, ii))^2;
    denominator = (Experimental_modes(:, ii)' * Experimental_modes(:, ii)) * (SDOF_modes(:, ii)' * SDOF_modes(:, ii));

    final_MAC(ii, :) = numerator / denominator;
end
